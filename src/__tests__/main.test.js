...
describe('Home', () => {
  test('it renders without crashing', () => {
    ...
    test('open item modal and do an action', () => {
      ...
      fireEvent.change(queryByTestId('modal-item-menu-input:' + id), {
        target: { value: 20 },
      })
      // The user changes the value of the description
      fireEvent.change(getAllByTestId('item-input-modal-descrip:' + id)[1], {
    ...
    test('use the add item modal', () => {
      ...
      fireEvent.change(queryByTestId('from-minutes'), {
        target: { value: '61' },
      })
      ...
      fireEvent.change(queryByTestId('until-hours'), {
        target: { value: '10' },
      })
      fireEvent.change(queryByTestId('until-minutes'), {
        target: { value: '00' },
      })
      // They change the end time from AM to PM
      fireEvent.change(queryByTestId('until-time-of-day'), {
        target: { value: 'PM' },
      })
